...
export function DnsCheckerContent() {
  const {
    rawInput,
    setRawInput,
    parsedDomains,
        ...
        <div className="p-5 space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs font-mono">
              <span className="text-muted-foreground">tld replace</span>
              <Input
                placeholder=".com"
                value={existingTld}
                onChange={(e) => {
                  setExistingTld(e.target.value);
                }}
                disabled={isLoading}
                className="h-7 w-20 px-2 text-xs font-mono bg-transparent"
              />
              ...
              {tldReplaceReady && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={handleReplaceTld}
                  disabled={isLoading || parsedDomains.length === 0}
