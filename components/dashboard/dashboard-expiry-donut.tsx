        ...
        <div className="flex flex-col lg:flex-row items-center gap-6">
                ...
                >
                  {chartData.map((d, _) => {
                    ...
                    return (
                      <Cell
                        key={d.name}
                        fill={COLORS[origIndex >= 0 ? origIndex : 0]}
          ...
          <div className="space-y-2 flex-1">
            {rawData.map((d, i) => {
              if (d.value === 0) {
                return (
                  <div key={d.name} className="flex items-center gap-2 text-xs text-text-muted/50 line-through">
                    <span className="w-3 h-3 rounded-sm shrink-0 opacity-30" style={{ backgroundColor: COLORS[i] }} />
                    <span className="w-16">{LABELS[i]}</span>
